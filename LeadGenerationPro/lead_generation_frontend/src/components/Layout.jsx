...
const COLORS = {
  ...
  inputBg: "rgba(73, 163, 196, 0.1)",
  gridLines: "rgba(0, 54, 74, 0.05)",

  shadowLight: "rgba(0, 54, 74, 0.15)",
  shadowButton: "rgba(0, 54, 74, 0.2)",

  spinnerBase: "rgba(255, 255, 255, 0.3)",
};
...
const Layout = ({ children, pageTitle }) => {
  ...
  const sidebarStyles = {
    ...
    sidebar: {
      ...
      color: COLORS.white,
      boxShadow: `10px 0px 20px ${COLORS.shadowLight}`,
      zIndex: 40,
      transition: "all 0.3s ease-in-out",
      width: sidebarOpen ? "280px" : "80px",
      overflow: "hidden",
          ...
          {Object.entries(menuContent).map(([key, { label, icon }]) => (
            <div key={key}>
              ...
              >
                ...
                {sidebarOpen && <span style={sidebarStyles.menuLabel}>{label}</span>}
                {sidebarOpen &&
                  (openMenu === key ? <ChevronDown size={20} /> : <ChevronRight size={20} />)}
              </button>
