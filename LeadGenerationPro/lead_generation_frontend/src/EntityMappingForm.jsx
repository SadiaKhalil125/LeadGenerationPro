  ...
  const fetchContent = async (urlToFetch) => {
    ...
    try {
      ...
      if (finalHtml.includes('<head>')) {
        finalHtml = finalHtml.replace(/<head[^>]*>/, `$&${baseTag}`);
      } else {
    ...
    <div className="w-full h-full flex flex-col">
      {status === 'idle' && (
        <div className="flex-grow flex flex-col items-center justify-center text-gray-500">
          ...
          <p>Enter a URL in the form to see a preview.</p>
        </div>
      )}
      {status === 'loading' && (
        <div className="flex-grow flex flex-col items-center justify-center text-gray-500">
          <div className="animate-spin text-teal-500"><Code size={48} /></div>
          <p className="mt-4 font-semibold">Fetching website content...</p>
        </div>
      )}
      {status === 'error' && (
        <div className="flex-grow flex flex-col items-center justify-center text-red-600 p-4">
          <AlertTriangle size={48} className="mb-4 text-red-500" />
          <h3 className="text-lg font-bold">Preview Failed</h3>
          <p className="text-center mt-2 bg-red-50 p-3 rounded-md">{errorMessage}</p>
        </div>
      )}
      {status === 'success' && (
        <iframe
